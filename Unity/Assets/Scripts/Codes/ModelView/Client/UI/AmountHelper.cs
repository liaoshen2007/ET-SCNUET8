...
namespace ET.Client
{
    ...
    public static class AmountHelper
    {
        private struct Num
        {
            public Num(string amount, int count)
            {
                this.Amount = amount;
                this.Count = count;
            }
        ...
        private const int numLangId = 1001;
        private const int wanLangId = 1002;
        private const int yiLangId = 1003;
        ...
        {
            Num num = Donum(Math.Abs(amount), 0);
            string re = num.Amount;
            switch (num.Count)
            {
                case 1:
                    re = $"{re}K";
        ...
        private static Num Donum(double amount, int count)
        {
            while (true)
            {
