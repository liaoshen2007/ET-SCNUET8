...
namespace ET.Client
{
    public class PrefabWin: EditorWindow
    {
        ...
        public void Reset()
        {
            ...
            if (mTab == 0)
            {
                ...
                foreach (string s in allAssets)
                {
                    ...
                    bool isComeFromPrefab = Regex.IsMatch(s, "Assets/UITools" + @"/((?!/).)*\.prefab");
                    if (isComeFromPrefab)
                    {
                        filtered.Add(s);
                    }
                ...
                {
                    this.AddGUID(AssetDatabase.AssetPathToGUID(s), -1);
                }
        ...
        private void GeneratePreview(Item item, bool isReCreate = true)
        {
        ...
        private void OnGUI()
        {
            ...
            if (mReset && type == EventType.Repaint)
            {
                ...
                foreach (Item item in mItems)
                {
                    this.GeneratePreview(item, false);
                }
