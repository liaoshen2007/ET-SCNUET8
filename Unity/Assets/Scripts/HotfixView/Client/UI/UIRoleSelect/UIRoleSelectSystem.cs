...
namespace ET.Client
{
    ...
    public static partial class UIRoleSelectSystem
    {
        ...
        public static async ETTask OnEnterGameClickHandler(this UIRoleSelect self)
        {
            ...
            {
                //bug et6的代码
                int errcode = await LoginHelper.LoginGameGate(self.Root(), account.AccountName, account.Password, account.Id, currentRoleId);
                ...
                errcode = await EnterMapHelper.EnterGameMap(self.Root(), currentRoleId);
                if (errcode != ErrorCode.ERR_Success)
                {
                    Log.Debug(errcode.ToString());
                    return;
